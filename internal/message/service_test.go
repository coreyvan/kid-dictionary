...
// mockMessageRepo implements message.Repository for testing.
type mockMessageRepo struct {
	createFn                      func(ctx context.Context, msg *message.Message) error
	getByConversationIDFn         func(ctx context.Context, conversationID uuid.UUID) ([]*message.Message, error)
	getRecentByConversationIDFn   func(ctx context.Context, conversationID uuid.UUID, limit int) ([]*message.Message, error)
}

func (m *mockMessageRepo) Create(ctx context.Context, msg *message.Message) error {
	if m.createFn != nil {
...
func (m *mockMessageRepo) GetByConversationID(ctx context.Context, conversationID uuid.UUID) ([]*message.Message, error) {
...
func (m *mockMessageRepo) GetRecentByConversationID(ctx context.Context, conversationID uuid.UUID, limit int) ([]*message.Message, error) {
...
// mockConversationRepo implements conversation.Repository for testing.
type mockConversationRepo struct {
	getByIDFn func(ctx context.Context, id uuid.UUID) (*conversation.Conversation, error)
}

func (m *mockConversationRepo) Create(ctx context.Context, conv *conversation.Conversation) error {
	return nil
}

func (m *mockConversationRepo) GetByID(ctx context.Context, id uuid.UUID) (*conversation.Conversation, error) {
	if m.getByIDFn != nil {
...
func (m *mockConversationRepo) List(ctx context.Context, userID *uuid.UUID, limit, offset int) ([]*conversation.Conversation, error) {
	return nil, nil
}

func (m *mockConversationRepo) Update(ctx context.Context, conv *conversation.Conversation) error {
	return nil
}
...
func TestService_SendMessage_FollowUpContext(t *testing.T) {
	...
	previousMessages := []*message.Message{
		{ID: uuid.New(), ConversationID: convID, Role: message.RoleUser, Content: "Why is the sky blue?"},
		{ID: uuid.New(), ConversationID: convID, Role: message.RoleAssistant, Content: "Light scatters!"},
	}

	msgRepo := &mockMessageRepo{
		getRecentByConversationIDFn: func(ctx context.Context, conversationID uuid.UUID, limit int) ([]*message.Message, error) {
			// Should include the new message plus history
			return append(previousMessages, &message.Message{
				ID:             uuid.New(),
	...
	convRepo := &mockConversationRepo{
		getByIDFn: func(ctx context.Context, id uuid.UUID) (*conversation.Conversation, error) {
			return &conversation.Conversation{
				ID:         convID,
				AgeBracket: conversation.AgeBracketGrowingMinds,
			}, nil
		},
	}
...
func TestService_SendMessage_AgeBracketVariation(t *testing.T) {
	testCases := []struct {
		name           string
		ageBracket     conversation.AgeBracket
...
func TestService_SendMessage_SensitiveTopic(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, message.ContentTierSensitive, result.AssistantMessage.ContentTier)
...
func TestService_SendMessage_ContextualTopic(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, message.ContentTierContextual, result.AssistantMessage.ContentTier)
...
func TestService_SendMessage_OffPurposeRequest(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, message.ContentTierRedirect, result.AssistantMessage.ContentTier)
...
func TestService_SendMessage(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		assert.Equal(t, message.RoleUser, result.UserMessage.Role)
		assert.Equal(t, "The sky is blue because of light!", result.AssistantMessage.Content)
		assert.Equal(t, message.RoleAssistant, result.AssistantMessage.Role)
	})
	...
	t.Run("error with empty content", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, message.ErrContentRequired)
	})
	...
	t.Run("error when conversation not found", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, message.ErrConversationNotFound)
	})
